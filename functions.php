...
if (ssh2_auth_password($ssh, check_user($olt), check_pw($olt))) {
    ...
    check_mode($ssh, $mode, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan, $pppoe, $profile);
} else {
    die('Gagal Terhubung');
}
...
function check_mode($ssh, $mode, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan, $pppoe, $profile) {
    if ($mode == "home") {
        home_config($ssh, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan, $pppoe, $profile);
    } elseif ($mode == "hotspot") {
        hotspot_config($ssh, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan);
    }
}

function home_config($ssh, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan, $pppoe, $profile){
    $s = ssh2_shell($ssh, 'vt102', null, 80, 24, SSH2_TERM_UNIT_CHARS);
    stream_set_blocking($s, true);
    
    $commands = [
        "conf t",
        "interface gpon-olt_1/$slot/$gpon",
        "onu $onuid type $type sn $sn",
        "exit",
        "interface gpon-onu_1/$slot/$gpon:$onuid",
        "name $nama",
        "sn bind enable",
        ...
        "exit",
        "pon-onu-mng gpon-onu_1/$slot/$gpon:$onuid",
...
function hotspot_config($ssh, $slot, $gpon, $onuid, $nama, $type, $sn, $vlan) {
    $s = ssh2_shell($ssh, 'vt102', null, 80, 24, SSH2_TERM_UNIT_CHARS);
    stream_set_blocking($s, true);
